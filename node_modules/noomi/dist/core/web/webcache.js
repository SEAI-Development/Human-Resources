...
class WebCache {
    ...
    static async load(request, response, url) {
        ...
        const rCheck = await this.check(request, url);
        switch (rCheck) {
            case 0:
                return 304;
            case 1:
    ...
    static async check(request, url) {
        ...
        const exist = await this.cache.has(url);
        if (!exist) {
            return 2;
        }
        // 检测 lastmodified
        const modiSince = request.getHeader('if-modified-since');
        if (modiSince) {
            const result = await this.cache.get(url, 'lastModified');
            if (modiSince !== result) {
                return 1;
            }
        }
        ...
        if (etag) {
            const result = await this.cache.get(url, 'etag');
            if (result !== etag) {
                return 1;
            }
        }
