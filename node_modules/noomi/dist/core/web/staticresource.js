...
class StaticResource {
    ...
    static async load(request, response, path, zip) {
        ...
        // 文件路径
        const filePath = util_1.Util.getAbsPath([path]);
        // 缓存数据对象
        let cacheData;
        // 状态码
        if (webconfig_1.WebConfig.useServerCache) { //从缓存取，如果用浏览器缓存数据，则返回0，不再操作
            const ro = await webcache_1.WebCache.load(request, response, path);
    ...
    static async readFile(path, zip) {
        ...
        // mime 类型
        const mimeType = application_1.App.mime.getType(path);
        // zip文件流
        let zipStream;
        const srcBufs = [];
        const zipBufs = [];
        let tmpFn;
        // 创建输入流
        const srcStream = fs.createReadStream(path);
        // 从源输入流读数据
        const srcBuf = await new Promise((res) => {
        ...
        // 不缓存数据标志
        const saveData = this.checkNeedZip(mimeType);
        if (zip && saveData) {
            // 生成临时文件
    ...
    static checkNeedZip(mimeType) {
        ...
        // 判断是否为可压缩类型
        for (const reg of this.zipTypes) {
