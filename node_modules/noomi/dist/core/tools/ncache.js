...
const noomierror_1 = require("./noomierror");
...
class NCache {
    ...
    constructor(cfg) {
         ...
         */
        this.redisSizeName = 'NCACHE_SIZE_';
        /**
         * redis存储前缀
         */
        this.redisPreName = 'NCACHE_';
        /**
         * timeout redis 前缀
         */
        this.redisTimeout = 'NCACHE_TIMEOUT_';
        // 如果为App为集群，则saveType为1，否则为设置值
        if (application_1.App.isCluster) {
            this.saveType = 1;
        }
        else {
            this.saveType = cfg.saveType || 0;
        }
        this.name = application_1.App.appName + '_' + cfg.name;
    ...
    async clearAll() {
        if (this.saveType === 0) {
            ...
            await redisfactory_1.RedisFactory.clear(this.redis);
        }
    }
    /**
     * 获取键数组
    ...
    async getKeys(key) {
        if (this.saveType === 0) {
            ...
            const client = redisfactory_1.RedisFactory.getClient(this.redis);
            if (client === null) {
                throw new noomierror_1.NoomiError("2601", this.redis);
            }
    ...
    async getFromRedis(key, subKey, changeExpire) {
        let timeout = 0;
        if (changeExpire) {
            const ts = await redisfactory_1.RedisFactory.get(this.redis, {
        ...
        const value = await redisfactory_1.RedisFactory.get(this.redis, {
    ...
    async getMapFromRedis(key, changeExpire) {
        ...
        const value = await redisfactory_1.RedisFactory.getMap(this.redis, {
            key: key,
            pre: this.redisPreName,
            timeout: timeout
        });
...
class MemoryCache {
        ...
        if (item.subKey) { // 子键
            if (ci.value) {
                ...
                if (typeof ci.value !== 'object') {
                    throw new noomierror_1.NoomiError("3010");
                }
            }
            else {
            ...
            const size = this.getRealSize(item.value);
            if (typeof item.value === 'object') {
                ci.size = size;
                // 新建value object
                if (!ci.value) {
     ...
     */
    getKeys(key) {
        const reg = util_1.Util.toReg(key);
        const arr = [];
    ...
    clearByLRU() {
        ...
        for (let i = 0; i < this.storeMap.size; i++) {
            ...
            if ((ind = indexes.indexOf(i)) !== -1) {
                const d = this.storeMap.get(k.value);
                delArr.push({
                    key: k.value,
                    lru: d.LRU,
                    size: d.size
    ...
    checkAndClean(item) {
        ...
        if (this.maxSize > 0 && size + this.size > this.maxSize) {
            ...
            if (size + this.size > this.maxSize) {
                throw new noomierror_1.NoomiError("3002");
            }
        }
    }
    /**
