...
class RedisFactory {
    ...
    static addClient(cfg) {
        const client = application_1.App.redis.createClient(cfg.port, cfg.host, cfg.options);
        client.on('error', err => {
            throw err;
        });
    ...
    static async set(clientName, item) {
        ...
        if (client === null) {
            throw new noomierror_1.NoomiError("2601", clientName);
        }
        if (!item.value) {
            throw new noomierror_1.NoomiError("3011");
        }
        // 合并pre
        const key = item.pre ? item.pre + item.key : item.key;
        // buffer 转 string
    ...
    static async setTimeout(client, key, timeout) {
        ...
        if (client === null) {
            throw new noomierror_1.NoomiError("2601", cn);
        }
     ...
     * @param subKey -        子键
     */
    static async del(clientName, key, subKey) {
        const client = this.getClient(clientName);
        if (client === null) {
            throw new noomierror_1.NoomiError("2601", clientName);
        }
    ...
    static parseFile(path) {
        ...
        try {
            ...
            throw new noomierror_1.NoomiError("2600") + '\n' + e;
        }
        this.init(json);
    }
    /**
